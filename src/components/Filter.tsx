...
interface FilterProps {
  ...
  onShowAll: () => void;
  currentCount: number; // Número de produtos exibidos
  totalCount: number;   // Número total de produtos
}

const Filter: React.FC<FilterProps> = ({ onFilterChange, onShowAll, currentCount, totalCount }) => {
  ...
  return (
    <div className='flex items-center h-[100px] w-screen bg-bannerColor gap-7' style={{ fontFamily: 'Poppins, sans-serif'}}>
      ...
      <p>Showing {currentCount}–{currentCount} of {totalCount} results</p>

      <div className='hidden md:flex md:items-center gap-4 md:ml-[497px]'>
        Show <div className='flex items-center justify-center h-14 w-14 bg-white text-gray-400'>{currentCount}</div>
